...
- **feature_gaps backfill (write-back):** the classifier could *suggest* `feature_gaps` checkbox values for AEs to confirm on the deal — closing the loop on the ~10% property-population problem. But that's a CRM mutation (`hubspot/` write-skill territory), out of scope for this read-only report.
